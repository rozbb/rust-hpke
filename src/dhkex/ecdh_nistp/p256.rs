...
impl Deserializable for PrivateKey {
    fn from_bytes(encoded: &[u8]) -> Result<Self, HpkeError> {
        ...
        let sk =
            p256::SecretKey::from_bytes(encoded.into()).map_err(|_| HpkeError::ValidationError)?;
...
impl DhKeyExchange for DhP256 {
    ...
    fn derive_keypair<Kdf: KdfTrait>(suite_id: &KemSuiteId, ikm: &[u8]) -> (PrivateKey, PublicKey) {
        super::nist_pxxx_derive::<32, Kdf, Self>(suite_id, ikm)
    }
}
...
mod tests {
    use crate::{
        dhkex::{ecdh_nistp::p256::DhP256, DhKeyExchange},
